...
const BRIDGE_AUDIO_CHUNK_BYTES = 96 * 1024; // 128 KiB after base64 encoding
...
function getBridgeState() {
  return {
    ...
    activeRequestId: pendingBridgeRequest?.requestId || activeBridgeTransfer?.request.requestId || null,
    phase: pendingBridgeRequest ? 'generating' : activeBridgeTransfer ? 'transferring' : null,
...
function postBridgeEvent(event) {
  ...
  document.documentElement.dataset.nghiTtsBridgeVersion = String(BRIDGE_VERSION);
  ...
  if (payload.type === 'cancelled' && payload.requestId) {
    document.documentElement.dataset.nghiTtsCancelledRequest = payload.requestId;
  }
  ...
  const logEntry = payload.type === 'audio_chunk'
...
async function sendAudioToFlutter(audioBlob, request) {
  ...
  const wasCancelled = () => (
    activeBridgeTransfer !== transfer || transfer.epoch !== bridgeOperationEpoch
  );
  ...
  try {
    ...
    const totalChunks = Math.ceil(bytes.length / BRIDGE_AUDIO_CHUNK_BYTES);
    ...
    for (let index = 0; index < totalChunks; index += 1) {
      ...
      const end = Math.min(start + BRIDGE_AUDIO_CHUNK_BYTES, bytes.length);
      postBridgeEvent({
        type: 'audio_chunk',
        requestId: request.requestId,
        index,
      ...
      // Yield between messages so long audio does not monopolize the WebView UI thread.
    ...
    bridgeError('AUDIO_TRANSFER_FAILED', err?.message || String(err), request.requestId);
  } finally {
...
function loadBridgeModel(modelName) {
  ...
  if (pendingBridgeRequest || activeBridgeTransfer || status.value === 'generating') {
  ...
  if (name === selectedModel.value && status.value === 'ready') {
    postBridgeEvent({ type: 'model_ready', model: name, voices: voices.value || [] });
...
function startSynthesisJob({ requestId, requestedText, normalizedVoice, normalizedSpeed, deliverToFlutter, autoPlay }) {
...
function synthesizeFromBridge(options = {}) {
  const requestedText = typeof options.text === 'string' ? options.text.trim() : '';
  const requestId = String(
    options.requestId || globalThis.crypto?.randomUUID?.() || `${Date.now()}-${Math.random().toString(36).slice(2)}`
  );
  ...
  if (!worker.value || status.value !== 'ready') {
    bridgeError('MODEL_NOT_READY', 'Wait for the model_ready event before synthesizing.', requestId);
    return { accepted: false, code: 'MODEL_NOT_READY', requestId };
  }
  if (pendingBridgeRequest || activeBridgeTransfer) {
    bridgeError('BUSY', 'Another synthesis request is still running.', requestId);
  ...
  const normalizedVoice = Number.isInteger(requestedVoice) && requestedVoice >= 0
    ? requestedVoice
    : 0;
...
function stopBridge(rawOptions = {}) {
  const options = typeof rawOptions === 'string'
    ...
    : (rawOptions || {});
  const requestedId = options.requestId == null ? null : String(options.requestId);
  ...
  const shouldReloadModel = phase === 'generating' && options.reloadModel !== false;
...
function installFlutterBridge() {
  ...
  document.documentElement.dataset.nghiTtsMethods = Object.keys(bridgeApi).join(',');
...
const restartWorker = (modelName = null) => {
  ...
  const nextWorker = new Worker(new URL("../workers/tts-worker.js", import.meta.url), {
    type: "module",
  });
...
const handlePlayPause = () => {
  if (status.value === "generating" && pendingBridgeRequest) {
    stopBridge({ requestId: pendingBridgeRequest.requestId, reloadModel: true });
    return;
  }
  ...
  switch (data.status) {
    ...
    case "error":
      ...
      if (pendingBridgeRequest) {
        bridgeError('GENERATION_FAILED', data.data, pendingBridgeRequest.requestId);
        pendingBridgeRequest = null;
      } else {
    ...
    case "complete":
      ...
      result.value = data.audio;
      {
        ...
        if (request?.deliverToFlutter && data.audio) {
          ...
          bridgeError('EMPTY_AUDIO', 'TTS completed without audio data.', request.requestId);
        }
        // Flutter owns bridge-generated files, so do not also duplicate them in
        // the browser's IndexedDB history.
        if ((!request || !request.deliverToFlutter) && data.audio && lastGeneration.value && selectedModel.value) {
          addEntry({
      ...
      <div class="p-6 pb-0 space-y-6">
        ...
        <div class="flex flex-col sm:flex-row gap-3">
          <button
            ...
            :disabled="(status === 'ready' && !isPlaying && !text) || (status !== 'ready' && status !== 'generating' && chunks.length === 0)"
          >
