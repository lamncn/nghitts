...
export class PiperTTS {
  ...
  async textToPhonemes(text) {
    ...
    const mergedPhonemeText = mergePhonemizerOutputPreservePunct(
      text,
      phonemes,
    );
    ...
    if (isDebugEnabled(config)) {
      console.log(
        `[PHONEMIZER] After marker removal: ${JSON.stringify(cleanedPhonemeText)}`,
      );
    }
  ...
  async phonemesToIds(textPhonemes) {
    ...
    if (isDebugEnabled(config)) {
      console.log(
        `[PHONEME TO ID] BOS=${idMap[BOS]}, PAD=${idMap[PAD]}, EOS=${idMap[EOS]}`,
      );
    }
    ...
    for (
      let sentenceIdx = 0;
      sentenceIdx < textPhonemes.length;
      sentenceIdx++
    ) {
      ...
      if (isDebugEnabled(config)) {
        console.log(
          `[SENTENCE ${sentenceIdx + 1}] Phonemes: ${JSON.stringify(sentencePhonemes)}`,
        );
      }
