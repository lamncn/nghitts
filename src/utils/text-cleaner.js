...
async function loadWordReplacementMap() {
  if (replacementMapCache !== null) {
    return replacementMapCache;
  }
  ...
  try {
    ...
    const sortedEntries = Array.from(replacementMap.entries()).sort(
      (a, b) => b[0].length - a[0].length,
    );

    // Create a new Map with sorted entries
    replacementMapCache = new Map(sortedEntries);
    return replacementMapCache;
  } catch (error) {
...
async function loadAcronymMap() {
  ...
  try {
    ...
    // Sort entries by length (longest first) and create a sorted array
    const sortedEntries = Array.from(acronymMap.entries()).sort(
      (a, b) => b[0].length - a[0].length,
    );
  ...
  while ((match = wordBoundaryRegex.exec(textCopy)) !== null) {
    ...
    if (word.length === 1) {
      if (isDebugEnabled(config)) {
        debugLog(config, "Transliteration", {
          word,
          action: "SKIPPED (single character)",
        });
      }
      continue;
    }
    ...
    result = result.replace(regex, (match, boundary, wordPart) => {
      const trans =
        wordPart && wordPart[0] === wordPart[0].toUpperCase()
 ...
 */
export async function replaceNonVietnameseWords(
  text,
  replacementMap,
  config = null,
) {
  ...
  for (const [original, transliteration] of replacementMap) {
    ...
    result = result.replace(regex, (match) => {
      ...
      if (match[0] === match[0].toUpperCase()) {
        return (
          transliteration.charAt(0).toUpperCase() + transliteration.slice(1)
        );
      }
      return transliteration;
    });
...
export function cleanTextForTTS(text) {
  ...
  const cleanedText = text
    // Removed characters must leave a separator so adjacent words cannot be
    // joined accidentally (for example: "anh—em" or "anh😀em").
    .replace(emojiRegex, " ")
    .replace(/…/g, "...")
    .replace(/[–—−]/g, " ")
    //.replace(/\b\/\b/, ' slash ')
...
export async function processTextForTTS(text) {
  ...
  const textAfterAcronymConversion = await convertAcronyms(
    mappingInput,
    acronymMap,
    config,
  );
  ...
  const textAfterWordReplacement = await replaceNonVietnameseWords(
    textAfterAcronymConversion,
    replacementMap,
    config,
  );
  if (isDebugEnabled(config)) {
  ...
  if (config.enableTransliteration) {
    processedText = applyTransliteration(
      textAfterWordReplacement,
      replacementMap,
      config,
    );
    if (isDebugEnabled(config)) {
...
const MAX_TTS_CHUNK_LENGTH = 500;

function splitLongTtsChunk(text, maxLength = MAX_TTS_CHUNK_LENGTH) {
  const result = [];
  ...
  while (remaining.length > maxLength) {
    ...
    const preferredBreak = Math.max(...candidates);
    const breakAt =
      preferredBreak >= Math.floor(maxLength * 0.5)
        ? preferredBreak + 1
        : maxLength;
...
export async function chunkText(text) {
  ...
  for (const line of lines) {
    ...
    for (const sentence of sentences) {
      ...
      if (trimmedSentence) {
        chunks.push(...splitLongTtsChunk(trimmedSentence));
      }
    }
  }
